...
[
    ...
    },
    {
        ...
        "z": "9f979da7e8a5400d",
        "group": "b35f5291aaa0dfd1",
        "name": "Fomula Pulse Main Speed",
        "label": "Fomula Pulse Main Speed",
        "order": 9,
        "width": "0",
        "height": "0",
        "maxrows": 0,
    ...
    },
    {
        ...
        "z": "9f979da7e8a5400d",
        "name": "function 32",
        "func": "let arr = new Array(24).fill(0);\nconst obj = {};\n\narr.forEach((element, index) => {\n    const h = String(index).padStart(2,0);\n    const key = `${h}:00`;\n    obj[key] = 0\n});\n\nconst items = msg.payload;\n\nitems.forEach(function(element) {\n    const h = element.create_at.substring(11, 13);\n    const key = `${h}:00`;\n    const fomula =(element.pulse_main_spd * 3.2) / 1_000;\n    const decimal = Math.trunc(fomula * 100) / 100;\n    obj[key] = decimal;\n});\nmsg.payload = obj;\nreturn msg;",
        "outputs": 1,
        "timeout": 0,
        "noerr": 0,
        "initialize": "",
    ...
    },
    {
        ...
        "z": "9f979da7e8a5400d",
        "name": "function 33",
        "func": "let arr = new Array(24).fill(0);\nconst obj = {};\n\narr.forEach((element, index) => {\n    const h = String(index).padStart(2,0);\n    const key = `${h}:00`;\n    obj[key] = 0\n});\n\nconst items = msg.payload;\n\nitems.forEach(function(element) {\n    const h = element.create_at.substring(11, 13);\n    const key = `${h}:00`;\n    const fomula =(element.pulse * 1.427) / 100;\n    const decimal = Math.trunc(fomula * 100) / 100;\n    obj[key] = decimal;\n});\nmsg.payload = obj;\nreturn msg;",
        "outputs": 1,
        "timeout": 0,
        "noerr": 0,
        "initialize": "",
    ...
    },
    {
        ...
        "z": "b79372892403a995",
        "name": "le",
        "func": "var length = msg.payload.length;\nlength = length - 1;\nglobal.set(\"config.state.influxdb.row\", length);\nif(length <= 1){\n    global.set(\"config.state.influxdb.index\", 0);\n};\nmsg.payload = {\n    fill: \"red\",\n    shape: \"dot\",\n    text: `row:${length}`};\nreturn msg;",
        "outputs": 1,
        "timeout": 0,
        "noerr": 0,
        "initialize": "",
    ...
    },
    {
        ...
        "z": "777823ab3e1fee97",
        "name": "",
        "func": "const pulse_main = shiftBit(msg.payload[0], msg.payload[1]);\nglobal.set(\"config.state.plc.pulse_main_spd\", pulse_main);\nnode.status({ fill: \"red\", shape: \"ring\", text: `${pulse_main}`});\nfunction shiftBit(l, h) {\n    return (h << 16) | l;\n}",
        "outputs": 1,
        "timeout": 0,
        "noerr": 0,
        "initialize": "",
